...
package com.xwc1125.droidutils.mms.pdu;
...
import com.xwc1125.droidutils.mms.ContentType;
import com.xwc1125.droidutils.mms.InvalidHeaderValueException;
import com.xwc1125.droidutils.mms.MmsException;
import com.xwc1125.droidutils.mms.util.DownloadDrmHelper;
import com.xwc1125.droidutils.mms.util.DrmConvertSession;
import com.xwc1125.droidutils.mms.util.PduCache;
import com.xwc1125.droidutils.mms.util.PduCacheEntry;
import com.xwc1125.droidutils.mms.util.SqliteWrapper;
...
public class PduPersister {
	...
	private static final String[] PDU_PROJECTION = new String[] { Mms._ID, Mms.MESSAGE_BOX, Mms.THREAD_ID,
			Mms.RETRIEVE_TEXT, Mms.SUBJECT, Mms.CONTENT_LOCATION, Mms.CONTENT_TYPE, Mms.MESSAGE_CLASS, Mms.MESSAGE_ID,
			Mms.RESPONSE_TEXT, Mms.TRANSACTION_ID, Mms.CONTENT_CLASS, Mms.DELIVERY_REPORT, Mms.MESSAGE_TYPE,
			Mms.MMS_VERSION, Mms.PRIORITY, Mms.READ_REPORT, Mms.READ_STATUS, Mms.REPORT_ALLOWED, Mms.RETRIEVE_STATUS,
			Mms.STATUS, Mms.DATE, Mms.DELIVERY_TIME, Mms.EXPIRY, Mms.MESSAGE_SIZE, Mms.SUBJECT_CHARSET,
	...
	private static final String[] PART_PROJECTION = new String[] { Part._ID, Part.CHARSET, Part.CONTENT_DISPOSITION,
			Part.CONTENT_ID, Part.CONTENT_LOCATION, Part.CONTENT_TYPE, Part.FILENAME, Part.NAME, Part.TEXT };
	 ...
	 * 
	 * <p>
	 * Title: getPduPersister
	 * </p>
	 * <p>
	 * Description: Get(or create if not exist) an instance of PduPersister
	 * </p>
	 * <p>
	 * 
	 * </p>
	...
	private PduPart[] loadParts(long msgId) throws MmsException {
		Cursor c = SqliteWrapper.query(mContext, mContentResolver, Uri.parse("content://mms/" + msgId + "/part"),
	...
	private void loadAddress(long msgId, PduHeaders headers) {
		Cursor c = SqliteWrapper.query(mContext, mContentResolver, Uri.parse("content://mms/" + msgId + "/addr"),
				...
				while (c.moveToNext()) {
					...
					if (!TextUtils.isEmpty(addr)) {
						...
						case PduHeaders.FROM:
							headers.setEncodedStringValue(new EncodedStringValue(c.getInt(1), getBytes(addr)),
									addrType);
							break;
						case PduHeaders.TO:
						case PduHeaders.CC:
						case PduHeaders.BCC:
							headers.appendEncodedStringValue(new EncodedStringValue(c.getInt(1), getBytes(addr)),
									addrType);
							break;
						default:
	...
	private void persistData(PduPart part, Uri uri, String contentType, HashMap<Uri, InputStream> preOpenedFiles)
		...
		try {
				...
				if (isDrm) {
					if (uri != null) {
						try {
							...
							if (len > 0) {
								// we're not going to re-persist and re-encrypt
								// an already
								// converted drm file
								return;
							}
			...
			if (drmConvertSession != null) {
				...
				// Reset the permissions on the encrypted part file so everyone
				// has only read
				// permission.
				File f = new File(path);
				ContentValues values = new ContentValues(0);
				SqliteWrapper.update(mContext, mContentResolver,
						Uri.parse("content://mms/resetFilePerm/" + f.getName()), values, null, null);
			}
		}
	}
	...
	private void updateAddress(long msgId, int type, EncodedStringValue[] array) {
		...
		SqliteWrapper.delete(mContext, mContentResolver, Uri.parse("content://mms/" + msgId + "/addr"),
				Addr.TYPE + "=" + type, null);
	 ...
	 * @tags @param uri
	 * @tags @param part
	 * @tags @param preOpenedFiles
	 * @tags @throws MmsException
	 * 
	 * @author xwc1125
	 * @date 2017年2月20日 上午11:52:09
	 */
	...
	public Uri persist(GenericPdu pdu, Uri uri, boolean createThreadId, boolean groupMmsEnabled,
		...
		// this scope.
		if ((msgType == PduHeaders.MESSAGE_TYPE_NOTIFICATION_IND) || (msgType == PduHeaders.MESSAGE_TYPE_RETRIEVE_CONF)
		...
		SqliteWrapper.update(mContext, mContentResolver, Uri.parse("content://mms/" + dummyId + "/part"), values, null,
				null);
	...
	private void loadRecipients(int addressType, HashSet<String> recipients,
		...
		// ??????????????????????????????????
		String myNumber = null;// excludeMyNumber ;//?
								// mTelephonyManager.getLine1Number() : null;
		for (EncodedStringValue v : array) {
			if (v != null) {
	...
	public Cursor getPendingMessages(long dueTime) {
		...
		return SqliteWrapper.query(mContext, mContentResolver, uriBuilder.build(), null, selection, selectionArgs,
