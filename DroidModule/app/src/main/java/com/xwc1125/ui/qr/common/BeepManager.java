...
public class BeepManager implements MediaPlayer.OnCompletionListener,
    ...
    private int beepResId = 0;
    private boolean vibrate;

    public BeepManager(Activity activity, boolean vibrate, int beepResId) {
    ...
    public synchronized void updatePrefs() {
        boolean isCanPlayBeep = shouldBeep(activity);
        if (beepResId > 0 && isCanPlayBeep && mediaPlayer == null) {
    ...
    public synchronized void playBeepSoundAndVibrate() {
        if (beepResId > 0 && mediaPlayer != null) {
            mediaPlayer.start();
        }
        if (vibrate) {
