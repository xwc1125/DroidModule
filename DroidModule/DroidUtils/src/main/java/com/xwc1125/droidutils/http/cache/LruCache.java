 ...
 * @version V1.0
 */
package com.xwc1125.droidutils.http.cache;
 ...
 * 
 * <p>
 * Title: LruMemoryCache
 * </p>
 * <p>
 * Description: LRU缓存
 * </p>
 * <p>
 * 使用LRU（最近最少使用）算法淘汰缓存数据，LRU算法基于LinkHashMap实现
 * </p>
 * 
 * @author zhangqy
 * @date 2016年5月6日上午10:40:55
 * 
 * @param <K>
 * @param <V>
 */
public class LruCache<K, V> {
	...
	private int maxSize;
	/**
	 * key: K value: 过期时间 从1970年1月1日0时开始
	 */
	...
	public LruCache(int maxSize) {
		...
		this.map = new LinkedHashMap<K, V>(0, 0.75f, true); // 参数1:容量大小 小于1时默认为1
															// 参数2:加载因子（用于扩充）
															// 参数3:true表示按照访问顺序迭代，false表示按照插入顺序迭代
	...
	public final V get(K key) {
		...
		synchronized (this) {
			...
			if (!keyExpiryMap.containsKey(key)) {// 过期
				this.remove(key);
				return null;
			}
		...
		synchronized (this) {
			mapValue = map.put(key, createdValue);// 返回oldValue

			if (mapValue != null) {// 有冲突，冲销上次插入
