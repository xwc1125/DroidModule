...
import com.xwc1125.droidutils.app.entity.ApkItemInfo;
import com.xwc1125.droidutils.LogUtils;
import com.xwc1125.droidutils.shell.ShellUtils;
 ...
 * <li>{@link PackageUtils#getInstallLocation()} get system install
 * location</li>
...
public class PackageUtils {
	...
	public static boolean isInstall(Context context, String packageName) {
		try {
			...
			LogUtils.e(TAG,e.getMessage(),e,isDebug);
		}
		return false;
	}
	...
	public static int installSilent(Context context, String filePath, String pmParams) {
		...
		LogUtils.e(TAG, new StringBuilder().append("installSilent successMsg:").append(commandResult.successMsg)
				.append(", ErrorMsg:").append(commandResult.errorMsg).toString(), isDebug);
	 ...
	 * manifest, so no need to request root permission, if you are system
	 * app.</li>
	 * </ul>
	 * 
	 * @param context
	 ...
	 *         <li>{@link #DELETE_FAILED_INTERNAL_ERROR} means internal
	 *         error</li>
	...
	public static int uninstallSilent(Context context, String packageName, boolean isKeepData) {
		...
		LogUtils.e(TAG, new StringBuilder().append("uninstallSilent successMsg:").append(commandResult.successMsg)
				.append(", ErrorMsg:").append(commandResult.errorMsg).toString(), isDebug);
		if (commandResult.errorMsg == null) {
	 ...
	 * 
	 * @author xwc1125
	 * @date 2016年8月18日 下午3:40:42
	 */
	public static ApkItemInfo getApkInfo(Context context, File apkFile) {
		try {
			ApkItemInfo apkItemInfo = new ApkItemInfo(context, apkFile);
			return apkItemInfo;
		} catch (Exception e) {
		} catch (Throwable tr) {
		}
	 ...
	 * 
	 * @tags @param context
	 * @tags @param packageName
	 * @tags @return
	 * 
	 * @author xwc1125
	 * @date 2016年8月22日 上午11:15:24
	 */
	public static ApkItemInfo getApkInfo(Context context, String packageName) {
		try {
			return new ApkItemInfo(context, packageName);
		} catch (NameNotFoundException e) {
		} catch (Exception e) {
		} catch (Throwable tr) {
		}
	 ...
	 * 
	 * @author xwc1125
	 * @date 2016年8月30日 上午11:58:59
	 */
	public static List<ApkItemInfo> getAllAppInfo(Context context) {
		ArrayList<ApkItemInfo> appList = new ArrayList<ApkItemInfo>(); // 用来存储获取的应用信息数据
		List<PackageInfo> packageInfos = context.getPackageManager().getInstalledPackages(0);
		for (PackageInfo packageInfo : packageInfos) {
			ApkItemInfo apkItemInfo = getApkInfo(context, packageInfo.packageName);
			if (apkItemInfo != null) {
				appList.add(apkItemInfo);
			}
		}
		return appList;
	}
